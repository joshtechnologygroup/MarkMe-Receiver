...
public class SyncActivity extends AppCompatActivity
    ...
    private Button mCallApiButton, syncButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        syncButton.setText(SYNC_BUTTON_TEXT);
        ...
        activityLayout.addView(syncButton);

        markMeDB = new MarkMeDB(this.getApplicationContext());
    ...
    private void sync() {
        if (! isGooglePlayServicesAvailable()) {
            ...
            new MakeSyncRequestTask(mCredential).execute();
        }
    }
    ...
    private boolean isGooglePlayServicesAvailable() {
        System.out.println("is play availaible");
     ...
     */
    private class MakeSyncRequestTask extends AsyncTask<Void, Void, List<String>> {
        private com.google.api.services.sheets.v4.Sheets mService = null;
        private Exception mLastError = null;
        private Spreadsheet spreadSheetData;
        private List<Sheet> attendanceSheets;
        private static final String ATTENDANCE_SHEET_ID = "1phyV8zz8OOHgXY_BmWgTqDe3r2umtsEdtGf436FcbsA";

        MakeSyncRequestTask(GoogleAccountCredential credential) {
        ...
        private List<List<Object>> getSheetData() {
            ...
            for (int i=0; i < actualData.size(); i++) {
                ...
                for (int j=0; j < actualData.get(i).size(); j++) {
                    objects.add(actualData.get(i).get(j));
                }
                data.add(objects);
            }
            return data;
